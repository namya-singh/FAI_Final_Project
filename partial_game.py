...
def simulate_partial_game(maze, agent_type="online_astar", pursuer_strategy="greedy",
    ...
    while step < step_limit:
        ...
        if dynamic:
            ...
            active_maze.step(protected_positions=protected)
            # update belief in agent's current visibility after shift
            pm.update_belief(agent_pos)

        # agent move — based only on belief map
        t0 = time.perf_counter()
        action, new_agent_pos = agent.step(agent_pos)
        ...
        p_action, new_pursuer_pos = pursuer.choose_move(game_state_for_pursuer)
